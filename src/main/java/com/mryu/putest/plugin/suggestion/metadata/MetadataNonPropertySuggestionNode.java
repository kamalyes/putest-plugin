package com.mryu.putest.plugin.suggestion.metadata;
...
import com.mryu.putest.plugin.suggestion.metadata.json.SpringConfigurationMetadataGroup;
...
/**
 * Represents a node in the hierarchy of suggestions Useful for navigating all suggestions & also acts as source of
 * truth. Note that some of the intermediate nodes might be there to just support the hierarchy
 * <p>
 * Also used for dot delimited search search. Each element corresponds to only a single section of the complete
 * suggestion hierarchy i.e if the we are building suggestions for
 * <ul>
    ...
    private SortedSet<Suggestion> addChildToMatchesAndSearchInNextLevel(Module module,
        ...
        for (MetadataSuggestionNode child : childNodes) {
            List<SuggestionNode> matchesRootTillChild =
                    unmodifiableList(GenericUtil.newListWithMembers(matchesRootTillParentNode, child));
